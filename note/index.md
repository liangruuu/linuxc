...
学完本章节内容之后希望能够实现自己的myls指令，如myls -a -l -i -n
...
FAT文件系统最大的缺陷是：1. 该文件系统所承载的文件数量是有一个上限的，原因就受制于结构体中的N值，而N值是有上限的，N决定了当前能够存放的文件个数；2. 单链表的设计，单链表最大的缺陷就是一个走向读取文件，假如说现在指针指向的是第二个data块，如果要找的数据在第一块的话，则无法通过单链表的形式读取第一块的内容. 虽然FAT文件系统又这么几个巨大缺陷，但因为其轻量级的设计，现在仍然有使用该系统的场合，比如U盘、SD卡...
